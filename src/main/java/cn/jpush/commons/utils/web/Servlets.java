/*
 * Copyright (c) 2015 www.rocyuan.com. All Rights Reserved.
 *
...
//	}
	...
	public static void writeResponse(HttpServletResponse response,
									 String result, String mediaType, int httpCode) {
		response.setContentType(mediaType);
		response.setStatus(httpCode); // 未授权
		PrintWriter write;
		try {
			write = response.getWriter();
			write.print(result);
	...
	public static void writeXMLResponse(HttpServletResponse response, String message) {
		response.setHeader("Cache-Control", "no-cache");
		response.setHeader("Pragma", "no-cache");
		response.setDateHeader("Expires", 0);
		response.setContentType(MediaTypes.APPLICATION_XML_UTF_8);
	...
	/**
	 * request参数转换成Map 去除空的参数
	 *
	 * @param request
	 * @return
	 */
	public static Map<String, String> filterEmptyRequestParameter(
